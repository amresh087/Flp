...
<h6>Start Date - <fmt:formatDate value="${test.startDate}" pattern="dd-MM-yyyy hh:mm a"/> <br /> End Date - <fmt:formatDate value="${test.endDate}" pattern="dd-MM-yyyy hh:mm a"/> </h6>
...
<a href="${pageContext.request.contextPath}/student/getQuestionAssessmentPage.htm?topicId=${tp.topic.id}&sId=${tp.subject.id}&subjectName=${tp.subject.name}&topicName=${tp.topic.name}&chapterId=${tp.chapter.id}">
<div class="student-board-container-inner">
<h6>${tp.topic.name} </h6>
...
<c:choose>
<c:when test="${not empty topic.topicSmartScore && topic.topicSmartScore.userId == sessionScope.user.id }">
GS ${topic.topicSmartScore.smartScore }
</c:when>
<c:otherwise>
GS 0
</c:otherwise>
